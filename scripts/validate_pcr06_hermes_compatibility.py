...
def _validate_semantics(contract: dict[str, Any]) -> None:
    ...
    denied = [
        "installation_authorized", "runtime_activation_authorized", "production_use_authorized",
        "provider_gateway_authorized", "oauth_authorized", "messaging_channels_authorized",
        "background_delegation_authorized", "autonomous_skill_writes_authorized",
        "autonomous_memory_writes_authorized", "yolo_mode_authorized", "real_client_data_enabled",
    ]
    _assert(all(adoption[key] is False for key in denied), "A denied Hermes capability was enabled")
    ...
    _assert(surfaces["HERMES-SURFACE-MEMORY"]["compatibility"] == "incompatible_for_canonical_state", "Hermes memory boundary drifted")
    ...
    _assert(worker["concurrency_limit"] == 1, "Worker concurrency escaped the boundary")
    required = {"background_fanout", "autonomous_merge", "direct_canonical_write", "external_send", "yolo_mode", "skill_write_without_review", "memory_write_without_review"}
    _assert(required <= set(worker["prohibited_actions"]), "Worker prohibitions are incomplete")
    ...
    expected = {"pcr03_merged_to_main", "pcr04_merged_to_main", "pcr05_merged_to_main", "pcr06_merged_to_main", "github_hosted_controls_in_issue_19_verified", "explicit_founder_phase_0_approval_received", "clean_macos_environment_available", "hermes_version_and_checksum_verified", "hermes_sandbox_conformance_passed"}
    _assert(set(contract["activation_conditions"]) == expected, "Hermes activation conditions drifted")
...
def _mutation_cases() -> list[tuple[str, Mutation]]:
    ...
    add(("adoption", "provider_gateway_authorized"), True, "enable gateway")
    add(("adoption", "background_delegation_authorized"), True, "enable background delegation")
    add(("adoption", "autonomous_skill_writes_authorized"), True, "enable skill writes")
    add(("adoption", "autonomous_memory_writes_authorized"), True, "enable memory writes")
    add(("adoption", "yolo_mode_authorized"), True, "enable yolo")
    add(("adoption", "real_client_data_enabled"), True, "enable client data")
    ...
    add(("upstream", "update_policy"), "automatic", "enable automatic updates")
    ...
    def map_mcp(value: dict[str, Any]) -> None:
        next(item for item in value["tool_mapping"] if item["hermes_capability"] == "mcp")["offdata_tool_classes"] = ["external_send"]
    def remove_prohibition(value: dict[str, Any]) -> None:
        value["worker_adapter"]["prohibited_actions"].remove("autonomous_merge")
    cases.extend([("remove sandbox conformance", remove_activation), ("map unreviewed MCP", map_mcp), ("remove autonomous merge prohibition", remove_prohibition)])
    return cases
...
def main() -> None:
    ...
    for token in ["PCR-06", "v0.18.2", "runtime_activation_authorized=false", "skill_manage", "background_fanout"]:
        _assert(token in documentation, f"PCR-06 documentation missing {token}")
    rejected = 0
    ...
    print(f"PCR-06 Hermes adoption and compatibility pack passed: {len(contract['compatibility_surfaces'])} surfaces, {len(contract['tool_mapping'])} capability mappings, {contract['readiness_snapshot']['repository_skill_count']} repository skills, {rejected} mutation cases rejected, local_prerequisites_passed=true, hermes_activation_authorized=false.")
