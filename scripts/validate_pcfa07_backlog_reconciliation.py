...
from pcfa07_backlog_reconciliation import RECORD, REPORT, TASK_RE, _mutate, build_records, decode, failures, run_self_test
...
def main() -> None:
    ...
    req(record == expected, 'PCFA-07 generated JSON does not match governed sources')
    req(REPORT.read_text(encoding='utf-8') == report, 'PCFA-07 evidence report drifted')
    req(not semantic(record), 'PCFA-07 semantic validation failed: ' + '; '.join(semantic(record)))
    req(not failures(state, record), 'PCFA-07 launch binding failed: ' + '; '.join(failures(state, record)))
    obs = decode(record)
    sets = source_sets()
    actual = {'PCFA-04': {x['requirement_id'] for x in obs if x['source_work_package'] == 'PCFA-04'}, 'PCFA-05-stages': {x['requirement_id'] for x in obs if x['obligation_kind'] == 'pcfa05_stage'}, 'PCFA-05-invariants': {x['requirement_id'] for x in obs if x['obligation_kind'] == 'pcfa05_invariant'}, 'PCFA-05-negative': {x['requirement_id'] for x in obs if x['obligation_kind'] == 'pcfa05_negative_case'}, 'PCFA-05-interrupts': {x['requirement_id'] for x in obs if x['obligation_kind'] == 'pcfa05_founder_interrupt'}, 'PCFA-06': {x['requirement_id'] for x in obs if x['source_work_package'] == 'PCFA-06'}}
    ...
    for pred, name in ((load_json(P4), 'PCFA-04'), (load_json(P5), 'PCFA-05'), (load_json(P6), 'PCFA-06')):
        contract = pred.get('pcfa07_reconciliation_contract', {})
        req(isinstance(contract, dict) and contract and all((v is True for v in contract.values())), f'{name} PCFA-07 reconciliation contract drifted')
    backlog = BACKLOG.read_text(encoding='utf-8')
    task_ids = set(TASK_RE.findall(backlog))
    req(len(task_ids) == 53, 'existing IMP task namespace drifted')
    req(not any((t.startswith('P0.') for x in obs for t in x['task_bindings'])), 'PCFA-07 widened IMP-P0')
    component_ids = {x['component_id'] for x in load_json(COMPONENTS)['integration_components']}
    req({c for x in obs for c in x['component_bindings']} <= component_ids, 'unknown component binding')
    handoff = load_json(HANDOFF)
    req(handoff['authority'].get('codex_implementation_backlog_reconciliation') == 'requirements/pcfa07-codex-implementation-backlog-reconciliation.json' and handoff['readiness'].get('pcfa07_implementation_backlog_reconciled') is True, 'current handoff omits PCFA-07')
    ...
    for path, tokens in ((BACKLOG, ('PCFA-07 reconciled corrective obligation overlay', '93 `planned_not_implemented` obligations', 'No PCFA-07 obligation is assigned to IMP-P0')), (DOC, ('Total reconciled obligations: **93**', 'Planned PCFA-07 implementation-test identities: **93**', 'No Phase 0 widening', 'PCFA-08', 'codex_start_authorized=false')), (STATUS, ('PCFA-07 Codex implementation backlog reconciliation', '93', 'PCFA07-TST-*', 'PCFA-08')), (ISSUE1, ('PCFA-07 Codex implementation backlog reconciliation', '93', 'planned_not_executed', 'P0.1–P0.4'))):
        ...
        [req(token in text, f'{path.relative_to(ROOT)} missing {token}') for token in tokens]
    ...
    for label, path, repl in cases:
        ...
        _mutate(v, path, repl)
        if list(validator.iter_errors(v)) or semantic(v) or failures(state, v):
            rejected += 1
        else:
            raise SystemExit(f'PCFA-07 mutation not rejected: {label}')
    launch = run_self_test(state)
    assignments = len({t for x in obs for t in x['task_bindings']})
