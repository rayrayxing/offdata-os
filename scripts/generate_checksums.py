...
def include(p):
 ...
 excluded_prefixes=('.git/','.github/','dist/','state/')
 return p.is_file() and s!='CHECKSUMS.sha256' and not s.startswith(excluded_prefixes) and '.env' not in p.name and 'private' not in p.name.lower() and '__pycache__' not in s and not s.endswith('.pyc')
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
