...
def _exact_map(registry: dict[str, Any]) -> dict[str, dict[str, Any]]:
    records = registry.get("exact_records", [])
...
def _classify(registry: dict[str, Any], relative: str) -> dict[str, Any] | None:
    ...
    for rule in registry.get("classification_rules", []):
        if isinstance(rule, dict) and fnmatch.fnmatchcase(relative, str(rule.get("pattern", ""))):
            matches.append(rule)
    if not matches:
        return None
    ...
    classes = {str(item.get("classification")) for item in winners}
    if len(classes) != 1:
...
def _semantic_failures(registry: dict[str, Any]) -> list[str]:
    ...
    require(len(read_order) >= 51, "combined authority read order regressed")
...
def main() -> None:
    ...
    structural_mutations.append(("duplicate rule", duplicate_rule))
    issue_one_stale = copy.deepcopy(registry)
    for item in issue_one_stale["external_records"]:
        if item["number"] == 1:
            item["current"] = False
    structural_mutations.append(("issue one demoted", issue_one_stale))
    issue_two_open = copy.deepcopy(registry)
    for item in issue_two_open["external_records"]:
        if item["number"] == 2:
            item["expected_state"] = "open"
    structural_mutations.append(("duplicate issue reopened", issue_two_open))

    for label, mutated in structural_mutations:
        if _semantic_failures(mutated):
            rejected += 1
        else:
