...
def _issue_read_order(path: Path) -> list[str]:
    ...
    if not items:
        raise ValueError('canonical issue body has no parseable required read order')
    return items
...
def _machine_read_order(path: Path) -> list[str]:
    ...
    if not isinstance(value, list) or not all((isinstance(item, str) for item in value)):
        raise ValueError('machine handoff read order is invalid')
    return list(value)
...
def _classify(registry: dict[str, Any], relative: str) -> dict[str, Any] | None:
    ...
    if len({str(item.get('classification')) for item in winners}) != 1:
        return {'classification': '__CONFLICT__', 'matches': winners}
    return winners[0]
...
def _semantic_failures(registry: dict[str, Any]) -> list[str]:
    ...
    expected_classes = {'handoff/codex-phase0-handoff.json': 'current_machine_handoff', 'handoff/codex-phase0-issue-final.md': 'current_issue_body', 'repository/canonical-authority-registry.json': 'current_authority_registry', 'contracts/workstream6-phase-namespace.json': 'current_phase_namespace', '.github/workflows/workstream6-final-pre-codex.yml': 'current_required_workflow', 'contracts/workstream6-required-workflow-identity.json': 'current_required_workflow_identity'}
    for path, classification in expected_classes.items():
        require(exact_by_path.get(path, {}).get('classification') == classification, f'current classification is invalid: {path}')
    try:
        sources = registry['read_order_sources']
        machine = _machine_read_order(ROOT / sources['machine_handoff'])
        issue = _issue_read_order(ROOT / sources['canonical_issue_body'])
    ...
    require(len(read_order) >= 52, 'combined authority read order regressed')
    ...
    current_issue_bodies = [p for p in issue_bodies if (_classify(registry, p.relative_to(ROOT).as_posix()) or {}).get('classification') == 'current_issue_body']
    ...
    current_machine_files = [p for p in machine_files if (_classify(registry, p.relative_to(ROOT).as_posix()) or {}).get('classification') == 'current_machine_handoff']
    ...
    for number, (classification, current, state) in expected_external.items():
        ...
        require(record.get('current') is current, f'issue #{number} current flag is invalid')
        require(record.get('expected_state') == state, f'issue #{number} expected state is invalid')
    ...
    require(isinstance(status, str) and 'WS6.6' in status, 'canonical WS6.6 status phrase is invalid')
    if isinstance(status, str):
        for relative in STATUS_FILES:
            text = (ROOT / relative).read_text(encoding='utf-8')
            require(status in text, f'canonical WS6.6 status phrase missing: {relative}')
    ...
    require(completion.get('next_permitted_work_package') == 'WS6.7', 'next package is not WS6.7')
...
def main() -> None:
    ...
    if errors:
        raise SystemExit('WS6.4 schema validation failed:\n- ' + '\n- '.join((error.message for error in errors)))
    ...
    cases = [(('work_package_id',), 'WS6.3'), (('base_main_sha',), '0' * 40), (('closed_defects',), ['WS6-CONSIST-001']), (('remaining_blocking_defects',), []), (('completion', 'all_required_prior_components_pass'), False), (('completion', 'ws64_complete'), False), (('completion', 'all_current_read_order_items_classified'), False), (('completion', 'all_evidence_roots_classified'), False), (('completion', 'final_reconciliation_complete'), True), (('completion', 'all_blocking_defects_closed'), True), (('completion', 'next_permitted_work_package'), 'WS6.8'), (('boundaries', 'founder_accountability_preserved'), False), (('boundaries', 'codex_start_authorized'), True), (('boundaries', 'phase0_implementation_authorized'), True), (('uniqueness_constraints', 'current_machine_handoff'), 2), (('uniqueness_constraints', 'current_issue_body'), 2), (('uniqueness_constraints', 'current_required_workflow'), 2), (('uniqueness_constraints', 'current_required_workflow_identity'), 2)]
    rejected = 0
    for path, replacement in cases:
        mutated = copy.deepcopy(registry)
        _set(mutated, path, replacement)
        if _semantic_failures(mutated):
            rejected += 1
        else:
            raise SystemExit(f"WS6.4 mutation was not rejected: {'.'.join(path)}")
    structural = []
    duplicate_exact = copy.deepcopy(registry)
    duplicate_exact['exact_records'].append(copy.deepcopy(duplicate_exact['exact_records'][0]))
    structural.append(('duplicate exact path', duplicate_exact))
    for missing, label in [('handoff/codex-phase0-handoff.json', 'missing current handoff'), ('.github/workflows/workstream6-final-pre-codex.yml', 'missing required workflow'), ('contracts/workstream6-required-workflow-identity.json', 'missing workflow identity contract')]:
        mutated = copy.deepcopy(registry)
        mutated['exact_records'] = [item for item in mutated['exact_records'] if item['path'] != missing]
        structural.append((label, mutated))
    promoted = copy.deepcopy(registry)
    for item in promoted['exact_records']:
        if item['path'] == 'handoff/codex-phase0-issue.md':
            item['classification'] = 'current_issue_body'
            item['current'] = True
    structural.append(('promoted superseded issue', promoted))
    missing_rule = copy.deepcopy(registry)
    missing_rule['classification_rules'] = [item for item in missing_rule['classification_rules'] if item['id'] != 'reports']
    structural.append(('unclassified reports', missing_rule))
    ...
    structural.append(('duplicate rule', duplicate_rule))
