...
TASK_RE = re.compile(r'^### (P(?:[0-9]|1[0-2])\.[0-9]+)\b', re.MULTILINE)
...
def _sources(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path, Path]:
    ...
    return (
        _json(ROOT / a['product_scope']),
        _json(ROOT / a['mvcl']),
        _json(ROOT / a['hermes_bounded_adoption']),
        ROOT / a['implementation_backlog'],
        ROOT / a['component_contract'],
    )
...
def _raw_obligations(config: dict[str, Any], p4: dict[str, Any], p5: dict[str, Any], p6: dict[str, Any]) -> list[tuple[str, str, list[str]]]:
    ...
    for item in p4['requirements']:
        rid = item['id']; kind = 'pcfa04_consulting_craft' if rid.startswith('CQ-') else 'pcfa04_requirement'
...
def build_records() -> tuple[dict[str, Any], str]:
    ...
    seen: set[str] = set(); test_ids: set[str] = set(); logical: list[dict[str, Any]] = []
    for rid, kind, bound in raw:
        ...
        logical.append({'requirement_id': rid, 'obligation_kind': kind, 'task_bindings': bound,
                        'source_work_package': 'PCFA-04' if kind.startswith('pcfa04_') else 'PCFA-05' if kind.startswith('pcfa05_') else 'PCFA-06',
                        'primary_implementation_task': bound[-1], 'blocking_phase_gate': _phase(bound[-1]),
                        'dependency_tasks': bound[:-1], 'component_bindings': comps, 'planned_test_id': test_id,
                        'evidence_type': c['evidence_type_by_obligation_kind'][kind], 'status': 'planned_not_implemented',
                        'implementation_evidence_status': 'not_available_pre_implementation'})
    counts = {'pcfa04_requirements': 29, 'pcfa05_stages': 19, 'pcfa05_invariants': 15, 'pcfa05_negative_cases': 13,
              'pcfa05_founder_interrupts': 6, 'pcfa06_capabilities': 11, 'total_obligations': 93, 'planned_tests': 93}
    actual = {k: sum(x['source_work_package'] == 'PCFA-04' for x in logical) if k == 'pcfa04_requirements' else
              sum(x['obligation_kind'] == {'pcfa05_stages':'pcfa05_stage','pcfa05_invariants':'pcfa05_invariant','pcfa05_negative_cases':'pcfa05_negative_case','pcfa05_founder_interrupts':'pcfa05_founder_interrupt','pcfa06_capabilities':'pcfa06_capability'}.get(k) for x in logical)
              for k in counts if k not in {'total_obligations','planned_tests'}}
    ...
    kind_meta = {kind: {'source_work_package': next(x['source_work_package'] for x in logical if x['obligation_kind'] == kind),
                        'evidence_type': c['evidence_type_by_obligation_kind'][kind]}
                 for kind in sorted({x['obligation_kind'] for x in logical})}
    ...
    record['source_digests'] = {'product_scope_sha256': _digest(ROOT / c['source_authorities']['product_scope']), 'mvcl_sha256': _digest(ROOT / c['source_authorities']['mvcl']),
                                'hermes_bounded_adoption_sha256': _digest(ROOT / c['source_authorities']['hermes_bounded_adoption']), 'implementation_backlog_sha256': _digest(backlog), 'component_contract_sha256': _digest(components)}
...
def decode(value: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    try:
        for row in value['obligations']:
            core = dict(zip(COLUMNS, row, strict=True)); tasks = core['task_bindings']; kind = core['obligation_kind']; comps = _uniq([c for t in tasks for c in task_map[t]])
            result.append(core | {'source_work_package':meta[kind]['source_work_package'],'primary_implementation_task':tasks[-1],'blocking_phase_gate':_phase(tasks[-1]),'dependency_tasks':tasks[:-1],
                                  'component_bindings':comps,'evidence_type':meta[kind]['evidence_type'],'status':defaults['status'],'implementation_evidence_status':defaults['implementation_evidence_status']})
    except (KeyError, TypeError, ValueError): return []
    return result


def failures(state: dict[str, Any], record: dict[str, Any] | None = None) -> list[str]:
    if not RECORD.is_file(): return ['PCFA-07 Codex implementation backlog reconciliation is missing']
    value = record or load_json(RECORD); out: list[str] = []
    def req(ok: bool, msg: str) -> None:
        if not ok: out.append(msg)
    try: expected, _ = build_records()
    except (KeyError, TypeError, ValueError) as e: return [f'PCFA-07 deterministic source reconstruction failed: {e}']
    ...
    obs = decode(value); tests = [x.get('planned_test_id') for x in obs]
...
def _mutate(value: dict[str, Any], path: tuple[str, ...], replacement: Any) -> None:
    if path[0] == 'obligations':
        row = value['obligations'][int(path[1])]; field = path[2]
        if field in COLUMNS: row[COLUMNS.index(field)] = replacement; return
        if field in {'status','implementation_evidence_status'}: value['obligation_defaults'][field] = replacement; return
        if field == 'evidence_type': value['kind_metadata'][row[COLUMNS.index('obligation_kind')]][field] = replacement; return
        if field == 'component_bindings': value['task_component_map'][row[COLUMNS.index('task_bindings')][0]] = replacement; return
        if field == 'dependency_tasks': row[COLUMNS.index('task_bindings')] = list(replacement) + row[COLUMNS.index('task_bindings')][-1:]; return
        if field == 'primary_implementation_task': row[COLUMNS.index('task_bindings')][-1] = replacement; return
    node: Any = value
    for part in path[:-1]: node = node[int(part)] if part.isdigit() else node[part]
    final = path[-1]
    if final.isdigit(): node[int(final)] = replacement
    else: node[final] = replacement


def run_self_test(state: dict[str, Any]) -> int:
    record = load_json(RECORD)
    if failures(state, record): raise SystemExit('PCFA-07 backlog reconciliation rejected: ' + '; '.join(failures(state, record)))
    ...
    for label,path,repl in cases:
        v=copy.deepcopy(state); _mutate(v,path,repl)
        if not failures(v,record): raise SystemExit(f'PCFA-07 launch-state mutation not rejected: {label}')
        rejected += 1
    ...
    for label,path,repl in cases2:
        ...
        if not failures(state,v): raise SystemExit(f'PCFA-07 launch-record mutation not rejected: {label}')
        rejected += 1
    return rejected
