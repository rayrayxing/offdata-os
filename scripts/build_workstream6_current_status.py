...
def build_records() -> tuple[dict[str, Any], str]:
    ...
    contract = {
        **source,
        ...
        "forbidden_pattern_count": len(source["forbidden_patterns"]),
        "status": "current_status_documents_reconciled",
    }
    report = "\n".join(
        [
            ...
            f"- Current authority surfaces: `{len(evidence)}`",
            f"- Forbidden stale-state patterns: `{len(source['forbidden_patterns'])}`",
            f"- Canonical status: {source['canonical_status_phrase']}",
...
def main() -> None:
    ...
    print(
        "Built WS6.3 current-status reconciliation: documents=6, closed_defects=2, "
